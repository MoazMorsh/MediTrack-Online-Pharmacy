...
let customerReviewList = [
    ...
    },
    {
        name: "Lina Sayed",
        date: "April 18, 2022",
        review: "Excellent experience. MediTrack has not let it's customers down during lockdown. Thanks team. Great job. The application is also very smooth. And does its job well with an attractive UI and easy to use features. Good job developer.",
    },
    {
        name: "Mohamed Ahmed",
        date: "April 21, 2022",
...
function createProductBannerList(list, container) {
    list.forEach(function (el){
        ...
        if (typeof el.discount == "number") {
            ...
            price.innerHTML = "<span>£" + el.EGP + "</span><br>" + "£" + el.price;
        } else {
...
function createProductList(list, container){
    list.forEach(function (el) {
        ...
        let img = document.createElement("img");
        let name = document.createElement("h4");
        let EGP = document.createElement("p");
        let price = document.createElement("h4");

        img.setAttribute("src", el.imgUrl);
        name.innerText = el.name;
        name.setAttribute("class", "name");
        EGP.innerHTML = "EGP <span>£" + el.EGP + "</span>";
        let p = (el.EGP / 100) * (100 - el.discount);
        p = (Math.round(p * 100) / 100).toFixed(2);
        price.innerHTML = "£" + p +  "<span>" + el.discount + "% OFF";
        div2.append(name, EGP, price);
        div.append(img, div2);
        container.append(div);
    })
}
